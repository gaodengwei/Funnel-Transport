...
if grad_level==-2 && nargout>p,
  error('geval:NonDifferentiable','Attempting to take the derivatives of a function that is not differentiable (grad_level==-2)');
end
  ...
  case 2  % taylorvar
    ...
    for i=1:length(varargin)
      if (~isobject(varargin{i}) || isa(varargin{i},'sym'))
        n= numel(varargin{i});%prod(size(varargin{i}));
    ...
    for i=1:p
      ...
      elseif (isa(f{i},'double'))
        ...
        for o=1:order  % return zeros for the gradients
          varargout{o*p+i}=zeros(numel(f{i}),ind^o);%zeros(prod(size(f{i})),ind^o);
        end
      else
  ...
  case 4  % symbolic
    ...
    for i=1:length(varargin)
      ...
      elseif (isnumeric(varargin{i}))
        ...
        for j=1:numel(varargin{i});%prod(size(varargin{i}))
