function y=getProjection(x,f,options)
...
if (~isfield(options,'x0')) options.x0 = [0;0]; end
...
X = zeros(length(options.x0),N);
if length(options.plotdims) == 2
  thetas = linspace(-pi,pi,N);
  X(options.plotdims,:) = [cos(thetas); sin(thetas)];
else
  X(options.plotdims,:) = randn(length(options.plotdims),N);
end
% For each direction, max. f'x subj. to x'Sx <= 1 is equivalent to
%                     max. f'x subj. to x'Sx == 1 ''
%                     max. f'S^(-1/2)x  subj. to y'y == 1
extrm = (Q\X)./repmat(sqrt(sum(X.*(Q\X))),length(options.x0),1);
y = repmat(d(options.plotdims),1,N) + extrm(options.plotdims,:);
