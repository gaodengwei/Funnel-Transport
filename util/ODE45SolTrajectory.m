classdef ODE45SolTrajectory < Trajectory
    % build by gao at 2017.12.4
    % this is a trajectory solved by ode45
    properties
        sol=[];
        shape;
    end
    ...
    methods
        function obj = ODE45SolTrajectory(sol,shape)
        ...
        function pptraj = flipToPT(obj)
            % trans to polyniminalTrajectory
            t = getBreaks(obj);
            ...
            if isempty(obj.shape)
                pptraj = polyniminalTrajectory(pchipDeriv(t,y,ydot));
            elseif iscell(obj.shape)
                offset=0;
                for i=1:length(obj.shape)
                    n=prod(obj.shape{i});
                    pptraj{i} = polyniminalTrajectory(pchipDeriv(t,y(offset + (1:n),:),ydot(offset + (1:n),:),ydot(offset + (1:n),:),obj.shape{i}));
                    offset = offset+n;
                end
            else
                pptraj = polyniminalTrajectory(pchipDeriv(t,y,ydot,ydot,obj.shape));
            end
        end
